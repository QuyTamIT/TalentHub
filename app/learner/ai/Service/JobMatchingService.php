...
final class JobMatchingService
{
    ...
    public function generate(string $studentId, string $requestId, string $idempotencyKey): array
    {
        ...
        if ($this->engine === null) return self::emptyResponse('provider_unavailable');
        ...
        catch (Throwable) { return self::emptyResponse('provider_unavailable'); }
        ...
        catch (Throwable) { try { $this->repository->failRun($studentId, (string) $pending['runId'], 'persistence_failure'); } catch (Throwable) {} return self::emptyResponse('provider_unavailable'); }
    ...
    {
        $candidateMap=[];foreach($candidates as $candidate)$candidateMap[$candidate->catalogId()]=$candidate;
        $groups=[];$topGap=null;
        foreach($run['items']??[] as $item){$id=(string)($item['catalogId']??'');$candidate=$candidateMap[$id]??null;if(!$candidate)continue;$meta=is_array($item['analysis']??null)?$item['analysis']:[];$ai=is_array($meta['analysis']??null)?$meta['analysis']:[];$enterprise=$candidate->enterpriseId()!==''?$candidate->enterpriseId():'provider:'.hash('sha256',$candidate->providerName());
            if (($run['state'] ?? '') === 'no_matching_jobs') {
                $gap=is_array($meta['skill_gap']??null)?$meta['skill_gap']:[];$gap['recommended_activities']=is_array($meta['recommended_activities']??null)?$meta['recommended_activities']:[];
                return ['state'=>'no_matching_jobs','analysis_origin'=>'gemini','score_origin'=>'deterministic_40_35_25','enterprise_groups'=>[],'near_match'=>['enterprise_id'=>$candidate->enterpriseId(),'enterprise_name'=>$candidate->providerName(),'catalog_id'=>$id,'title'=>$candidate->title(),'url'=>$candidate->canonicalUrl(),'match_score'=>(int)$item['matchScore'],'match_tier'=>'low_fit','score_breakdown'=>$meta['score_breakdown']??[],'analysis'=>$ai['analysis']??'','strength_skill_codes'=>$ai['strength_skill_codes']??[],'gap_skill_codes'=>$ai['gap_skill_codes']??[],'gap_explanations'=>$ai['gap_explanations']??[],'evidence_ref_ids'=>$ai['evidence_ref_ids']??[]],'skill_gap'=>$gap,'run_id'=>$run['runId']??null,'freshness_status'=>$stale?'stale':'fresh'];
            }
            ...
            $groups[$enterprise]['positions'][]=['catalog_id'=>$id,'title'=>$candidate->title(),'url'=>$candidate->canonicalUrl(),'match_score'=>(int)$item['matchScore'],'match_tier'=>$meta['score_breakdown']['tier']??'developing_fit','score_breakdown'=>$meta['score_breakdown']??[],'analysis'=>$ai['analysis']??'','strength_skill_codes'=>$ai['strength_skill_codes']??[],'gap_skill_codes'=>$ai['gap_skill_codes']??[],'gap_explanations'=>$ai['gap_explanations']??[],'evidence_ref_ids'=>$ai['evidence_ref_ids']??[]];
            if($topGap===null){$topGap=is_array($meta['skill_gap']??null)?$meta['skill_gap']:[];$topGap['recommended_activities']=is_array($meta['recommended_activities']??null)?$meta['recommended_activities']:[];}
        }
