...
final class DatabaseRoadmapRepository implements RoadmapRepository
{
    ...
    {
        $statement = $this->pdo->prepare('SELECT roadmaps.*, runs.engineType, runs.ruleVersion, runs.provider, runs.modelVersion, runs.promptVersion, runs.fallbackReason FROM learner_ai_roadmaps AS roadmaps INNER JOIN learner_recommendation_runs AS runs ON runs.id = roadmaps.runId WHERE roadmaps.id = :roadmapId AND roadmaps.studentId = :studentId');
