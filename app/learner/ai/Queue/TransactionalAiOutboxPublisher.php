...
final class TransactionalAiOutboxPublisher
{
 ...
 public static function publish(PDO $pdo,string $aggregateType,string $aggregateId,int $aggregateVersion,array $studentIds,string $eventType,array $safePayload=[],?string $tenantId=null):bool
 {
  ...
  if(!self::tableExists($pdo,'learner_ai_data_outbox')){if(strtolower((string)(getenv('TALENTHUB_AI_ENABLED')?:'false'))==='true'&&$pdo->getAttribute(PDO::ATTR_DRIVER_NAME)!=='sqlite')throw new \RuntimeException('AI outbox schema is required while AI is enabled.');return false;}
