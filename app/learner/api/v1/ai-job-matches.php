...
try {
    ...
    if ($request->method === 'GET') {
        ...
        JsonResponder::sendSuccess($context->jobMatchingService($studentId)->latest($studentId), $context->requestId());
    }
    ...
    $beforeGenerate = static fn () => (new PersistentActionRateLimiter($context->pdo()))->consume('learner.ai', $studentId, isset($_SERVER['REMOTE_ADDR']) ? (string) $_SERVER['REMOTE_ADDR'] : null);
    $result = $context->jobMatchingService($studentId)->generate($studentId, $context->requestId(), $idempotencyKey, $beforeGenerate);
    JsonResponder::sendSuccess(
        $result,
        $context->requestId(),
        ($result['state'] ?? null) === 'pending' ? 202 : 200,
    );
    ...
    JsonResponder::sendError(new ApiException(503, 'SERVICE_UNAVAILABLE', 'Dịch vụ AI gợi ý việc làm tạm thời không khả dụng.'), $context?->requestId() ?? 'request-unavailable');
}
