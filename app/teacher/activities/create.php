...
$pageTitle = $action === 'edit' ? 'Chỉnh sửa hoạt động' : 'Tạo hoạt động mới';
$formHeading = $pageTitle;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($csrfValid && in_array($formAction, ['registration_transition', 'advance_status', 'submit_for_school_review'], true)) {
        $redirectId = $postedActivityId !== '' ? $postedActivityId : $activityId;
        if ($redirectId !== '') {
            header('Location: detail.php?id=' . rawurlencode($redirectId));
            exit;
        }
    ...
    if (!$csrfValid) {
        ...
        $formValues = [
            ...
            'skillIds' => array_values(array_filter(array_map(
                static fn ($id): string => strtolower(trim((string) $id)),
                is_array($_POST['skillIds'] ?? null) ? $_POST['skillIds'] : []
            ))),
        ...
        if ($ownedEditActivity !== null) {
            $editFallbacks = [
                ...
                'skillIds' => $ownedEditActivity['assigned_skill_ids'] ?? [],
        ...
        if (!is_array($formValues['skillIds'] ?? null) || $formValues['skillIds'] === []) {
            $addFieldError('skillIds', 'Vui lòng chọn ít nhất một kỹ năng cho hoạt động.');
        }
    if (!$errors) {
        $payload = [
            ...
            'experienceHighlights' => $formValues['experienceHighlights'],
        ...
        try {
            if ($formAction === 'edit') {
                ...
                header('Location: detail.php?id=' . rawurlencode($postedActivityId) . '&saved=updated');
                exit;
            }
            if ($formAction !== 'create') {
                throw new \TalentHub\Http\ApiException(422, 'VALIDATION_FAILED', 'Thao tác hoạt động không hợp lệ.');
            }
            $newActivityId = $activityService->create($teacherId, $schoolId, $payload);
            header('Location: detail.php?id=' . rawurlencode($newActivityId) . '&saved=created');
            exit;
                ...
                <div class="teacher-container">
                    <section class="teacher-activities-heading">
                        <div>
                            ...
                            <h2 class="teacher-activities-heading__title"><?= teacherActivitiesEscape($formHeading); ?></h2>
                            <p class="teacher-activities-heading__description">
                                <a href="index.php">← Quay lại danh sách</a>
                                <?php if ($action === 'edit' && $activityId !== ''): ?>
                                    · <a href="detail.php?id=<?= teacherActivitiesEscape($activityId); ?>">Xem chi tiết</a>
                                <?php endif; ?>
                            </p>
                        </div>
                    </section>
                        ...
                        <section class="teacher-section-box teacher-activities-form-panel">
                            <div class="teacher-section-box__header">
                                ...
                                <div class="teacher-activities-header-actions">
                                    ...
                                    <?php if ($action === 'edit' && $activityId !== ''): ?>
                                        <a href="detail.php?id=<?= teacherActivitiesEscape($activityId); ?>" class="btn btn-secondary btn-sm">Xem chi tiết</a>
                                    <?php endif; ?>
                                </div>
                            </div>
                                ...
                                <section class="teacher-form-block teacher-activities-form__section" aria-labelledby="activity-experience-heading">
                                    ...
                                    <div class="teacher-form-grid-2col">
                                        <div class="teacher-form-col">
                                            ...
                                            <div class="teacher-form-field teacher-form-field--wide<?= isset($fieldErrors['skillIds']) ? ' has-error' : ''; ?>">
                                                <span>Kỹ năng phát triển <small>(chọn từ catalog — dùng khi chấm điểm)</small></span>
                                                <?php if ($skillCatalog === []): ?>
                                                    <p class="teacher-form-hint" role="alert">Chưa có kỹ năng active trong catalog.</p>
                                                <?php else: ?>
                                                    <?php
                                                    $selectedSkillIds = array_map('strval', is_array($formValues['skillIds'] ?? null) ? $formValues['skillIds'] : []);
                                                    $skillsByCategory = [];
                                                        ...
                                                        <div class="teacher-skill-tree__panel" role="tree" aria-label="Kỹ năng phát triển">
                                                            <?php foreach ($skillsByCategory as $category => $skills):
                                                                ...
                                                                foreach ($skills as $skill) {
                                                                    if (in_array((string) $skill['id'], $selectedSkillIds, true)) {
                                                                        $categoryChecked++;
                                                                    }
                                                                }
                                                            ...
                                                            ?>
                                                                <div class="teacher-skill-tree__node<?= $startOpen ? ' is-open' : ''; ?>" data-skill-tree-node role="treeitem" aria-expanded="<?= $startOpen ? 'true' : 'false'; ?>" data-category="<?= teacherActivitiesEscape($categoryKey); ?>">
                                                                    <div class="teacher-skill-tree__row teacher-skill-tree__row--parent">
                                                                        <button type="button" class="teacher-skill-tree__toggle" data-skill-tree-toggle aria-label="Mở/đóng nhóm <?= teacherActivitiesEscape($category); ?>">
                                                                        ...
                                                                        <label class="teacher-skill-tree__check">
                                                                            <input type="checkbox" data-skill-tree-parent <?= $allChecked ? 'checked' : ''; ?> <?= $someChecked ? 'data-indeterminate="1"' : ''; ?>>
                                                                            <span><?= teacherActivitiesEscape(mb_strtoupper($category, 'UTF-8')); ?></span>
                                                                        </label>
                                                                    </div>
                                                                    <div class="teacher-skill-tree__children" role="group"<?= $startOpen ? '' : ' hidden'; ?>>
                                                                        <?php foreach ($skills as $skill):
                                                                            $skillId = (string) $skill['id'];
                                                                            $checked = in_array($skillId, $selectedSkillIds, true);
                                                                            $skillName = (string) $skill['name'];
                                                                        ?>
                                                                            <label class="teacher-skill-tree__row teacher-skill-tree__row--child" data-skill-tree-leaf data-skill-name="<?= teacherActivitiesEscape(mb_strtolower($skillName, 'UTF-8')); ?>">
                                                                                <span class="teacher-skill-tree__check">
                                                                                    <input type="checkbox" name="skillIds[]" value="<?= teacherActivitiesEscape($skillId); ?>" <?= $checked ? 'checked' : ''; ?>>
                                                                                    <span><?= teacherActivitiesEscape($skillName); ?></span>
                                                                                </span>
                                                                            </label>
                                                                        <?php endforeach; ?>
                                                                    </div>
                                                ...
                                                <?php endif; ?>
                                                <?php if (isset($fieldErrors['skillIds'])): ?>
                                                    <p class="teacher-form-error" role="alert"><?= teacherActivitiesEscape($fieldErrors['skillIds']); ?></p>
                                                <?php endif; ?>
                                            </div>
