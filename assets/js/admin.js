(() => {
  ...
  const refreshDashboard = async () => {
    try {
      ...
      document.querySelectorAll('[data-nav-count]').forEach((badge)=>{const matched=visibleQueue.find((item)=>item.type===badge.dataset.navCount);const count=badge.dataset.navCount==='dashboard'?queueCount:Number(matched?.count||0);badge.textContent=count.toLocaleString('vi-VN');badge.hidden=count===0;badge.setAttribute('aria-label',`${count} mục cần xử lý`);});
