    ...
    function createOpportunityMatchController({ api, view, createIdempotencyKey }) {
            ...
            if (state === 'ready-model' || state === 'stale-model' || state === 'low-fit-model' || hasAnalyzedNoFitItems) {
                ...
                if (!items) {
                    if (lastRenderablePayload) {
                        view.render('stale-model', { ...lastRenderablePayload, state: 'stale_model' });
                    } else {
                        view.render('source-error', { items: [] });
                    }
                    return;
                }
                const renderable = { ...payload, items };
                lastRenderablePayload = renderable;
                view.render(state, renderable);
                return;
            }
            if (state === 'source-error' && lastRenderablePayload) {
                view.render('stale-model', { ...lastRenderablePayload, state: 'stale_model' });
                return;
            }
        ...
        async function load() {
            ...
            try {
                ...
                renderResponse({ state: 'provider_unavailable', items: [] });
            }
        }
