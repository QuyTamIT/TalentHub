...
rollout_assert($visible['state'] === 'ready_model' && array_map(static fn (RecommendationResult $result): string => $result->engineType(), $repository->completed) === ['rule', 'model'], 'model visibility completes a rule fallback before a validated assigned model run');
