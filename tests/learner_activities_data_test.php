...
activity_assert(
    str_contains($myActivitiesSource, '$canCancel = in_array($status, [\'pending\', \'approved\', \'waitlisted\'], true)')
        && str_contains($myActivitiesSource, '$now < $cancellationClose')
        && str_contains($myActivitiesSource, 'data-cancel-registration=')
        && str_contains($activityJavascript, 'if(serverMutationsEnabled){')
        && str_contains($activityJavascript, "await gateway.cancel(registration.id,'Sinh viên chủ động hủy')")
        && str_contains($activityJavascript, '}else if(localMutationsEnabled){'),
    'server-rendered cancellation observes policy and database mode uses the authenticated gateway'
);
activity_assert(
    !str_contains($myActivitiesSource, 'data-feedback-registration')
        && !str_contains($activityJavascript, "querySelectorAll('[data-feedback-registration]')"),
    'registered page does not expose browser-local feedback controls'
);

$activities = learner_activity_catalog();
$mockCatalog = learner_activity_mock_catalog();
$registrations = learner_activity_registration_history('student-demo-001');
activity_assert(count($mockCatalog) >= 6, 'mock source retains six deterministic activity fixtures');
activity_assert(count(array_unique(array_column($activities, 'id'))) === count($activities), 'activity ids are unique');

$modes = [];
foreach ($mockCatalog as $activity) {
...
foreach ($activities as $activity) {
    ...
    $close = new DateTimeImmutable((string) $activity['registration_closes_at']);
    activity_assert($start > new DateTimeImmutable('now'), 'mock discovery excludes activities that already started');
    activity_assert($close > new DateTimeImmutable('now'), 'mock discovery excludes closed registration windows');
    $ownStatuses = array_column(array_filter(
        $registrations,
        static fn (array $registration): bool => ($registration['activity_id'] ?? null) === $activity['id'],
    ), 'status');
    activity_assert(array_intersect($ownStatuses, ['pending', 'approved', 'waitlisted', 'attended']) === [], 'mock discovery excludes own active registrations');
}
