...
$dcr = str_replace("\r\n", "\n", (string) file_get_contents($dcrPath));
...
$mysqlSql = str_replace("\r\n", "\n", implode("\n\n", $migration->statements('mysql')));
$sqliteSql = str_replace("\r\n", "\n", implode("\n\n", $migration->statements('sqlite')));
...
recommendation_schema_assert($mysqlSql === $dcrSql, 'MySQL statements exactly reproduce approved DCR fence');
recommendation_schema_assert(hash('sha256', $mysqlSql) === RECOMMENDATION_STORE_DDL_SHA256, 'MySQL statement fingerprint is approved');
recommendation_schema_assert($sqliteSql === $dcrSqlite, 'SQLite statements exactly reproduce approved DCR fence');
recommendation_schema_assert(!str_contains($mysqlSql, 'CREATE TABLE IF NOT EXISTS'), 'recommendation migration refuses pre-existing targets instead of accepting an unchecked shape');
