...
foreach ([
    'Dự án mà bạn đã hoàn thành tại doanh nghiệp là minh chứng cho năng lực.',
    'Các dự án bạn đã triển khai tại công ty ABC thể hiện năng lực.',
    'Bạn có nhiều dự án triển khai tại doanh nghiệp.',
    'Dựa trên bạn đã thực tập tại công ty ABC, bạn có kinh nghiệm thực tế.',
] as $historyClaim) {
    $rejects(fn () => $guard->assertText($historyClaim, $facts), 'Do not strip personal history from an artifact or evidence prefix.');
}
...
foreach ([
    ...
    'Viết kịch bản kiểm thử cho các API đã xây dựng để đảm bảo tính ổn định.',
    'Tín hiệu điểm số ghi nhận mức độ thành thạo trong lập trình Python và phân tích logic. Bạn có thể phát huy thế mạnh này qua việc cấu trúc mã nguồn dự án rõ ràng và tối ưu quy trình xử lý dữ liệu.',
    'Bạn có cơ hội tham gia các bài tập nhóm và dự án nhỏ trong giai đoạn tiếp theo.',
] as $text) {
