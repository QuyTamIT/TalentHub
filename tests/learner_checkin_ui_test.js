...
function loadClient({ mediaDevices, detector, jsQR, canvas, send, history = [], videoPlay } = {}) {
  ...
  const nodes = {
    ...
    submit: createNode(), reset: createNode(), history: createNode(), historyAction: createNode(),
  };
  nodes.historyAction.hidden = true;
  ...
  const selectors = {
    ...
    '[data-checkin-history-action]': nodes.historyAction,
  };
  ...
  const sandbox = {
    document: {
      ...
      createElement(tag) { return tag === 'canvas' && canvas ? canvas : createNode(); },
      addEventListener() {},
    },
...
test('successful check-in reveals the activity-history action without changing the QR workflow', async () => {
  ...
  assert.match(page, /activity-history\.php/, 'history action targets the learner attendance history route');

  const client = loadClient();
  assert.equal(client.nodes.historyAction.hidden, true, 'history action starts hidden until the server confirms attendance');
  await client.contract.submitToken('opaque-success-token');
  assert.equal(client.nodes.historyAction.hidden, false, 'only a successful API response reveals the history action');
});

test('activity-history action is visually hidden until the server confirms check-in', () => {
  const page = fs.readFileSync(path.join(__dirname, '..', 'app', 'learner', 'checkin.php'), 'utf8');
  const client = fs.readFileSync(path.join(__dirname, '..', 'assets', 'js', 'learner-checkin.js'), 'utf8');

  assert.match(page, /data-checkin-history-action\s+hidden\s+style="display:\s*none;"/);
  assert.match(client, /historyAction\.style\.removeProperty\(['"]display['"]\)/);
});
...
test('jsQR fallback decodes canvas pixels and submits the scanned token', async () => {
  let stopped = 0;
  let drawCalls = 0;
    ...
    getContext(type, options) {
      ...
      return {
        drawImage(video, x, y, width, height) {
          drawCalls += 1;
          assert.equal(video.videoWidth, 2);
  ...
  const client = loadClient({
    ...
    jsQR(data, width, height, options) {
      jsQrCalls.push({ data, width, height, options });
      return { data: 'fallback-camera-token' };
    },
  });
  ...
  assert.equal(jsQrCalls.length, 1);
  assert.equal(jsQrCalls[0].data, pixels.data);
  assert.equal(jsQrCalls[0].width, 2);
  assert.equal(jsQrCalls[0].height, 2);
  assert.equal(jsQrCalls[0].options.inversionAttempts, 'attemptBoth');
  assert.equal(client.apiCalls.send.length, 1);
  assert.equal(client.apiCalls.send[0].body.token, 'fallback-camera-token');
  assert.ok(stopped >= 1, 'successful fallback scan stops the media track');

  const page = fs.readFileSync(pagePath, 'utf8');
  const vendorIndex = page.indexOf('../../assets/js/vendor/jsQR.js');
  const clientIndex = page.indexOf('../../assets/js/learner-checkin.js');
  assert.ok(vendorIndex >= 0, 'check-in page loads the vendored jsQR browser build');
  assert.ok(vendorIndex < clientIndex, 'jsQR must load before the learner check-in client');
});

test('vendored jsQR decodes a real upstream RGBA raster fixture', () => {
  const fixturePath = path.join(__dirname, 'fixtures', 'jsqr-upstream-end-to-end-0.rgba');
  assert.equal(fs.existsSync(fixturePath), true, 'upstream jsQR raster fixture is available locally');
  const raster = fs.readFileSync(fixturePath);
  assert.equal(raster.length, 300 * 200 * 4, 'fixture contains a complete 300x200 RGBA raster');

  const vendoredJsQR = require('../assets/js/vendor/jsQR.js');
  const pixels = new Uint8ClampedArray(raster.buffer, raster.byteOffset, raster.byteLength);
  const decoded = vendoredJsQR(pixels, 300, 200, { inversionAttempts: 'attemptBoth' });

  assert.ok(decoded, 'vendored decoder locates the real QR raster');
  assert.equal(decoded.data, 'Happy 1st Birthday GeekUp Leeds! http://www.geekup.org');
});
