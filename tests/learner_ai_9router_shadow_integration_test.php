...
$geminiResponse = $geminiProvider->generate($request);
...
$directResponse = $directProvider->generate($request);
...
$timeoutResponse = $timeoutProvider->generate($request);
...
test_assert($http400->generate($request)->errorCode() === 'provider_rejected', '400 produces provider_rejected');
...
test_assert($http401->generate($request)->errorCode() === 'provider_rejected', '401 produces provider_rejected');

// 3.3 HTTP 403 Forbidden
$http403 = new HttpRecommendationProvider($config, static fn (): array => ['status' => 403, 'headers' => [], 'body' => 'Forbidden']);
test_assert($http403->generate($request)->errorCode() === 'provider_rejected', '403 produces provider_rejected');
...
test_assert($http500->generate($request)->errorCode() === 'provider_unavailable', '500 produces provider_unavailable');
...
$res502 = $http502->generate($request);
...
test_assert($malformedText->generate($request)->errorCode() === 'malformed_response', 'non-JSON body is malformed_response');
...
test_assert($brokenJson->generate($request)->errorCode() === 'malformed_response', 'broken JSON is malformed_response');
...
test_assert($missingItems->generate($request)->errorCode() === 'malformed_response', 'missing items key is malformed_response');

// 4.4 Non-array items
$scalarItems = new HttpRecommendationProvider($config, static fn (): array => ['status' => 200, 'headers' => [], 'body' => '{"items": "string-not-array"}']);
test_assert($scalarItems->generate($request)->errorCode() === 'malformed_response', 'non-array items is malformed_response');
...
test_assert(($qualityResponse['state'] ?? '') === 'insufficient_data', 'quality gate failure returns insufficient_data state without invoking provider');
...
test_assert(($assessmentOnlyResponse['state'] ?? '') === 'insufficient_data', 'service returns insufficient_data for assessment-only input');
