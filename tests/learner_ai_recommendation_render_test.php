...
roadmap_render_assert(!str_contains($page, 'learner-recommendations.js'), 'legacy recommendation renderer is removed');
roadmap_render_assert(str_contains($client, 'textContent'), 'untrusted model strings use textContent');
roadmap_render_assert(!str_contains($client, 'innerHTML'), 'renderer never assigns HTML from model output');
roadmap_render_assert(!str_contains($page . $client, 'TALENTHUB_AI_API_KEY'), 'API key is never rendered');
