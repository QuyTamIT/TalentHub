...
try {
    ...
    $expectedInstructionInsights = 'insights phải có đúng ba record, mỗi record dùng duy nhất một trong ba category: strength, improvement, potential; mỗi category xuất hiện đúng một lần. Không dùng trùng lặp category.';
    $assert(
        in_array($expectedInstructionInsights, $instructions, true),
        'Instructions must specify exactly three insights with unique categories.'
    );

    // Never reassign a model claim to a different semantic category.
    $payloadWithDuplicateCategory = $mockPayload;
    $payloadWithDuplicateCategory['insights'][2]['category'] = 'strength'; // duplicate!
    try {
        $validator->fromProviderPayload($payloadWithDuplicateCategory, $engineMetadata);
