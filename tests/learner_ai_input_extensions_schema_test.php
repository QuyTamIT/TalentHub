...
ai_extensions_assert($mysqlSql === $dcrSql, 'MySQL statements exactly reproduce approved DCR fence');
ai_extensions_assert(hash('sha256', $mysqlSql) === AI_INPUT_EXTENSIONS_DDL_SHA256, 'MySQL statement fingerprint is approved');
ai_extensions_assert(!str_contains($mysqlSql, 'CREATE TABLE IF NOT EXISTS'), 'extension migration refuses pre-existing targets instead of accepting an unchecked shape');
