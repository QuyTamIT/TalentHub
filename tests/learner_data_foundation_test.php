...
$database->exec('CREATE TABLE talent_tests (id TEXT PRIMARY KEY, code TEXT, name TEXT, type TEXT, status TEXT)');
$database->exec('CREATE TABLE test_questions (id TEXT PRIMARY KEY, testId TEXT, code TEXT, content TEXT, optionsJson TEXT, status TEXT)');
$database->exec('CREATE TABLE test_attempts (id TEXT PRIMARY KEY, testId TEXT, studentId TEXT, status TEXT, startedAt TEXT, submittedAt TEXT)');
$database->exec('CREATE TABLE test_results (id TEXT PRIMARY KEY, attemptId TEXT, resultCode TEXT, summary TEXT, dimensionScoresJson TEXT, scoringVersion TEXT, createdAt TEXT)');
...
$insert = $database->prepare('INSERT INTO talent_tests (id, code, name, type, status) VALUES (:id, :code, :name, :type, :status)');
$insert->execute(['id' => $ids['assessment'], 'code' => 'holland_high', 'name' => 'Holland', 'type' => 'holland', 'status' => 'published']);
$insert = $database->prepare('INSERT INTO test_questions (id, testId, code, content, optionsJson, status) VALUES (:id, :testId, :code, :content, :optionsJson, :status)');
$insert->execute(['id' => $ids['question'], 'testId' => $ids['assessment'], 'code' => 'H1', 'content' => 'Câu hỏi database', 'optionsJson' => json_encode([1, 2, 3, 4, 5]), 'status' => 'published']);
$insert = $database->prepare('INSERT INTO test_attempts (id, testId, studentId, status, startedAt, submittedAt) VALUES (:id, :testId, :studentId, :status, :startedAt, :submittedAt)');
$insert->execute(['id' => $ids['attempt'], 'testId' => $ids['assessment'], 'studentId' => $ids['student'], 'status' => 'submitted', 'startedAt' => '2026-08-14 08:00:00', 'submittedAt' => '2026-08-14 08:10:00']);
$insert = $database->prepare('INSERT INTO test_results (id, attemptId, resultCode, summary, dimensionScoresJson, scoringVersion, createdAt) VALUES (:id, :attemptId, :resultCode, :summary, :dimensionScoresJson, :scoringVersion, :createdAt)');
$insert->execute(['id' => $ids['result'], 'attemptId' => $ids['attempt'], 'resultCode' => 'RIA', 'summary' => 'Kết quả database', 'dimensionScoresJson' => json_encode(['R' => 90]), 'scoringVersion' => 'holland-riasec-1.0', 'createdAt' => '2026-08-14 08:10:00']);
...
foundation_assert($databaseDefinition['code'] === 'holland_high', 'canonical assessment code is readable');
foundation_assert($databaseDefinition['status'] === 'published', 'canonical assessment status is readable');
...
foundation_assert($databaseAttempts[0]['status'] === 'submitted', 'submittedAt derives submitted status');
