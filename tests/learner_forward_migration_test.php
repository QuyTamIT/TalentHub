...
try {
    ...
    $canonicalChecksumFixture = <<<'PHP'
<?php
return new \TalentHub\Learner\Data\Migrations\ForwardMigrationDefinition(
    '003_canonical_checksum', 'Canonical source checksum', __FILE__,
    hash('sha256', str_replace("\r\n", "\n", (string) file_get_contents(__FILE__))),
    new class implements \TalentHub\Learner\Data\Migrations\LearnerForwardMigration {
        public function version(): string { return '003_canonical_checksum'; }
        public function description(): string { return 'Canonical source checksum'; }
        public function statements(string $driver): array { return []; }
        public function expectedSchema(): array { return []; }
    }
);
PHP;
    file_put_contents($directory . '/003_canonical_checksum.php', str_replace("\n", "\r\n", $canonicalChecksumFixture));
    forward_assert(isset($runner->status()['003_canonical_checksum']), 'runner accepts a platform-neutral migration checksum');
    unlink($directory . '/003_canonical_checksum.php');
