...
try {
    ...
    $allowed = static fn(string $id): bool => $config->enabled()
        && $config->shadowGateApproved()
        && $config->visiblePercent() > 0
        && !$config->pilotPaused()
        && trim((string) $config->pilotApprovalReference()) !== ''
        && $policy->isAssigned($id, $config);

    $store = new DatabaseSchoolAiInsightRepository($pdo);
    $circuit = new CircuitBreaker(3, 30, null, new DatabaseCircuitBreakerStore($pdo), 'gemini-school-insight');
    $explainer = new SchoolAiGeminiExplainer($config, null, null, $circuit);
    $service = new SchoolAiInsightService(
        new SchoolAiAggregateRepository($pdo),
        static fn(string $id): array => ['id' => $id],
        $explainer,
        static fn(string $id): ?array => $store->latest($id),
        5,
        static fn(string $id, array $payload) => $store->save($id, $payload, (string) ($config->model() ?? 'unknown')),
        $allowed,
        $config->model(),
        true
    );
    $queue = new DatabaseSchoolAiRefreshJobRepository($pdo);
    $worker = new SchoolAiRefreshWorker($queue, $service);
    $worker->runOnce();
} catch (\Throwable $e) {
    fwrite(STDERR, "School AI refresh worker failed safely.\n");
    exit(78);
}
