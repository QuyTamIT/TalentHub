...
try {
    $testing=in_array('--testing',$argv,true);$env=Environment::appEnvironment();
    ...
    try{(new RolePermissionSeeder())->run($pdo);if($testing){(new MinimalAuthRbacSeeder())->run($pdo,$env,Environment::required(MinimalAuthRbacSeeder::PASSWORD_ENV));}}
    finally{$release=$pdo->prepare('SELECT RELEASE_LOCK(?)');$release->execute(['talenthub:system_seeds']);}
    fwrite(STDOUT,'[OK] system seed'.PHP_EOL);if($testing){fwrite(STDOUT,'[OK] testing seed'.PHP_EOL);}exit(0);
