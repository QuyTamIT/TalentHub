...
require dirname(__DIR__).'/Database/seeds/Demo/CompletePortalDemoSeeder.php';
...
use TalentHub\Database\Seeds\Demo\CompletePortalDemoSeeder;
try {
    $testing=in_array('--testing',$argv,true);$demo=in_array('--demo',$argv,true);$completeDemo=in_array('--complete-demo',$argv,true);$env=Environment::appEnvironment();
    ...
    if($completeDemo&&!in_array($env,['local','test'],true)){throw new RuntimeException('Complete demo seed is allowed only in local/test.');}
    ...
    try{
        ...
        if($completeDemo){(new CompletePortalDemoSeeder())->run($pdo,$env,Environment::required(CompletePortalDemoSeeder::PASSWORD_ENV));}
    }
    ...
    if($completeDemo){fwrite(STDOUT,'[OK] complete demo seed'.PHP_EOL);}
    exit(0);
