...
final class SchoolAiAggregateRepository
{
    ...
    /** @return array{ready:bool,error_code:?string} */
    ...
    public function aggregate(string $schoolId, int $minimumCohort = 5, ?string $retentionCutoff = null): array
    {
        ...
        foreach ($statement->fetchAll(PDO::FETCH_ASSOC) as $row) {
            if (!$this->evidenceFresh((string) ($row['evidence_json'] ?? ''), $cutoff)) {
                continue;
            }
            $gen = (string) ($row['generated_at'] ?? '');
            if ($gen !== '' && ($sourceMaxUpdatedAt === null || $gen > $sourceMaxUpdatedAt)) {
                $sourceMaxUpdatedAt = $gen;
            }
            ...
            foreach ([[$classKey, 'class', (string) $row['class_name']], [$gradeKey, 'grade', (string) $row['grade_level']], ['school:' . $schoolId, 'school', 'Toàn trường']] as [$key, $level, $label]) {
                ...
                if ((string) $row['status'] === 'stale_model') {
                    $groups[$key]['stale_count']++;
                }
        ...
        foreach ($groups as $group) {
            if ($group['student_count'] < $minimumCohort) {
                $suppressed++;
                continue;
            }
            $talents = [];
            foreach ($group['talent_totals'] as $field => $total) {
                if (($group['talent_counts'][$field] ?? 0) < $minimumCohort) {
                    continue;
                }
                $talents[] = [
                    'field' => $field,
                    'average_score' => round($total / $group['talent_counts'][$field], 1),
                ];
            }
            ...
            foreach ($group['trend_counts'] as $label => $count) {
                if ($count < $minimumCohort) {
                    continue;
                }
                $trends[] = [
                    'label' => $label,
                    'count' => $count,
                    'confidence' => round($count / $group['student_count'], 2),
                ];
            }
    ...
    public function aggregateHash(array $aggregate): string
    {
        unset($aggregate['generated_at']);
        return hash('sha256', json_encode($aggregate, JSON_THROW_ON_ERROR | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES));
    }
    ...
    private function evidenceFresh(string $value, string $cutoff): bool
    {
        if (trim($value) === '') {
            return true;
        }
        try {
            ...
            foreach ($items as $item) {
                foreach (['observed_at', 'updated_at', 'generated_at', 'occurred_at'] as $key) {
                    $timestamp = $item[$key] ?? null;
