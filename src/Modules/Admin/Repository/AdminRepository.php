...
final class AdminRepository
{
    ...
    public function organizations(string $type='', string $search=''): array
    {
        ...
        if($this->tableExists('organization_registration_requests')){
            ...
            $request=$this->pdo->prepare("SELECT id,organizationName AS name,type,'pending' AS status,'pending' AS verificationStatus,createdAt,expiresAt,email,schoolLevel,1 AS registrationRequest FROM organization_registration_requests WHERE {$where} ORDER BY createdAt DESC LIMIT 200");
    ...
    {
        if(!in_array($type,['school','enterprise'],true)||!in_array($decision,['verified','rejected','pending','suspended'],true)){throw new RuntimeException('Quyết định xác minh không hợp lệ.');}
