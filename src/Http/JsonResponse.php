...
final class JsonResponse
{
    ...
    public function __construct(public readonly int $status, public readonly array $payload, public readonly string $requestId) {}
    ...
    { $error=['code'=>$e->errorCode,'message'=>$e->getMessage()];if($e->details!==[]){$error['details']=$e->details;}return new self($e->status,['error'=>$error,'meta'=>self::meta($requestId)],$requestId); }

    public function send(): never
    {
        http_response_code($this->status);header('Content-Type: application/json; charset=utf-8');header('X-Request-Id: '.$this->requestId);header('Cache-Control: no-store');
